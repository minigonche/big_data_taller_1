...
class ClasificadorSingleton:
    """
    Clase de ETL

    Se maneja como singleton para facilitar las pruebas y direccionar
    programaticamente a archivos de pruebas. El patrón singleton solo se
    utiliza con este fin dentro de la ejecucion
    """

    __instance = None

    def __new__(cls):
        if ClasificadorSingleton.__instance is None:
            ClasificadorSingleton.__instance = Clasificador()

        return ClasificadorSingleton.__instance
...
class Clasificador:
    ...
    def __init__(self, pickle_loc = "app2/Requerimientos/clasificador/clasificadores_entrenados",
                        data_loc = 'app2/Requerimientos/clasificador/data/datos_polaridad.csv'):

        self.polaridad = SentimentClassifier()

        #Sexismo
        with open(pickle_loc + '/SEXISMO_machine.pkl', "rb") as input_file:
            maquina_sexismo = pickle.load(input_file)

            data_propia = pd.read_csv(data_loc, sep=',', encoding = "ISO-8859-1")
            data_propia = data_propia[['TEXTO','SEXISMO']]
            data_propia.columns = ['TEXTO','CATEGORIA']
            data_propia.dropna(inplace = True)

            self.sexismo = EncapsularClasificador(maquina_sexismo)
            self.sexismo.fit(data_propia['TEXTO'].values.tolist(), data_propia['CATEGORIA'].values.tolist())


        with open(pickle_loc + '/MATONEO_machine.pkl', "rb") as input_file:
            maquina_matoneo = pickle.load(input_file)

            data_propia = pd.read_csv(data_loc, sep=',', encoding = "ISO-8859-1")
            data_propia = data_propia[['TEXTO','MATONEO']]
            data_propia.columns = ['TEXTO','CATEGORIA']
            data_propia.dropna(inplace = True)
...
class EntrenarModelo:
    ...
    def configurar_modelo(X, y, idioma = 'SPANISH', fitness = accuracy_score):
        ...
        if('linear' in option_machines):
            ...
            for i in range(num_ite):
                ...
                #Divide los datos
                X_train, X_test, y_train, y_test = EntrenarModelo.balanced_split(X_svm, y, test_size=0.2)
        ...
        if('bayes' in option_machines):
            ...
            for i in range(num_ite):
                ...
                #Divide los datos
                X_train, X_test, y_train, y_test = EntrenarModelo.balanced_split(X_mnb, y, test_size=0.2)
        ...
        for i in range(num_ite):
            ...
            X_train, X_test, y_train, y_test = EntrenarModelo.balanced_split(X, y, test_size=0.2)
    ...
    def construir(categoria):
        ...
        data_propia = pd.read_csv('app2/Requerimientos/clasificador/data/datos_polaridad.csv', sep=',', encoding = "ISO-8859-1")
        ...
        clf = EntrenarModelo.configurar_modelo(data_propia['TEXTO'].values.tolist(), data_propia['CATEGORIA'].values.tolist(), idioma = "SPANISH")

        with open('app2/Requerimientos/clasificador/clasificadores_entrenados/' + categoria + '_machine.pkl', 'wb') as output:
...
if __name__ == "__main__":
    ...
    print(categoria)
    EntrenarModelo.construir(categoria)
